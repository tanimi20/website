...
const Navbar = () => {
  ...
  // Determine background and text/icon colors based on scroll and route
  ...
  const iconColor = isHomePage || isScrolled ? "text-gray-900" : "text-white";
            ...
            {navItems.map((item, index) => (
              <li
                ...
                onMouseEnter={() => {
                  if (item.submenu) {
                    ...
                    setActiveSubMenu(item.submenu[0].label); // Set default submenu
                  }
                }}
                onMouseLeave={() => {
                  if (item.submenu) {
              ...
              >
                ...
                {item.submenu && isServicesOpen && (
                  <div className="absolute left-1/2 -translate-x-1/2 mt-3 w-96 bg-white shadow-xl rounded-xl overflow-hidden transition-all duration-300 flex border border-gray-100">
                    {/* Left: India/UAE menu */}
                    <ul className="w-1/2 border-r border-gray-200 bg-gray-50">
                      {item.submenu.map((sub, subIndex) => (
                        <li
                          key={subIndex}
                          className={`px-6 py-3 cursor-pointer text-sm font-medium hover:bg-blue-100 transition-colors ${
